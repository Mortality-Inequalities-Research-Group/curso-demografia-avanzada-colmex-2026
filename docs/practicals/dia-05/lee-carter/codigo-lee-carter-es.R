...
##
##  Modelando y proyectando mortalidad con el método de Lee-Carter method
##
...
source("funciones-tabla-vida.R")

## loading England & Wales data
load("lee-carter-data.RData")

## dimensions of matrices
initial.year <- 1970
#which column is 1970
initial.year.ind <- initial.year - 1959
#total number of colums
last.year.ind <- dim(LMX)[2]

LMX <- LMX[,initial.year.ind:last.year.ind]
E <- E[,initial.year.ind:last.year.ind]
Y <- Y[,initial.year.ind:last.year.ind]

t <- t[t>= initial.year]
t1 <- t1[t1>= initial.year]
...
## plotting observed log-mortality [HW ggplot]
par(mar=c(3,3,1,1))
...
#[HW optional -> how do i create a GIF with tidyverse?]
for(i in 1:n1){
...
## observed life expectancy [HW ggplot]
...
## fitted life expectancy [HW ggplot]
