# Curso de Demografía Avanzada, El Colegio de México, 2026
# Funciones auxiliares para la práctica del Día 4

# Etiquetas de las causas de muerte
cause_names <- c(
  "1" = "Infecciosas y respiratorias",
  "2" = "Neoplasias",
  "3" = "Circulatorias y cardiacas",
  "4" = "Perinatales",
  "5" = "Diabetes",
  "6" = "Homicidios",
  "7" = "Otras causas externas",
  "8" = "Otras",
  "9" = "Mal definidas"
)

# Funciones compatibles con DemoDecomp
...
e0.frommxc <- function(mxcvec, age, sex = NULL, ax = NULL){
  ...
  mx <- rowSums(mxcvec)
  e0.frommx(mx = mx, age = age, sex = sex, ax = ax)
}

# Disparidad de la duración de la vida
...
edagger.frommxc <- function(mxcvec, age, sex = NULL, ax = NULL){
  dim(mxcvec) <- c(length(age), length(mxcvec)/length(age))
  mx <- rowSums(mxcvec)
  edagger.frommx(mx = mx, age = age, sex = sex, ax = ax)
}
